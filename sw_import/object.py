...
from io_soulworker.chunks.mtrl_chunk import MtrlChunk
...
class ImportObject(VisChunkFile):
    ...
    def process_mtrs(self, chunk: int, reader: BufferedReader):
        ...
        for _ in range(count):
            v_material = MtrlChunk(reader, self.path)

            material = bpy.data.materials.new(v_material.name)
            ...
            #     )
            # else:
            path: Path = self.path.parent / v_material.diffuse_map
            ...
            if v_material.ui_transparency_type != VisTransparencyType.NONE:
